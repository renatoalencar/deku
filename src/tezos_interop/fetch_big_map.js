...
const finished = (map_contents) => output({ status: "success", map_contents });
const error = (error) =>
  output({ status: "error", error: JSON.stringify(error) });

(async () => {
  const { rpc_node, contract_address, confirmation, validators } = input();
  ...
  const res = await Promise.all(
    validators.map(validator =>
      client.getBigMapKey(
        contract_address,
        { "key": { "string": validator }, "type": { "prim": "key_hash" } },
        { block: block.hash }
      ).then(value => [validator, value]))
  );
