...
let test_incorrect_syntax () =
  let code = "(module" in
  Alcotest.check_raises "Raises parse error" Parse_error
    (fun () -> ignore (parse code))
  
let test_memory_size () =
  let code = "(module (memory 65540))" in
  Alcotest.check_raises "Raises validation error" Parse_error
    (fun () -> ignore (parse code))
...
let test =
  ...
  "Parsing", [ test_case "Successful parsing" `Quick test_successful_parsing
             ...
             ; test_case "Contract validation (single memory)" `Quick test_multiple_memories
