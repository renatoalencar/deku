...
let invoke ?imports ~storage ~argument code =
  ...
  let runtime = Wasm_vm.Runtime.make ~contract ~imports in
...
let test_simple_invocation () =
  ...
  Alcotest.(check Hexdump.hex) "Same" (Hexdump.of_string "2B 00 00 00 00 00 00 00") storage
...
let test_extern_bindings () =
  ...
  let opaque_fn _ _ =
    called := true;
    Some (Wasm_vm.Value.i64 13L)
  in
  let imports = Wasm_vm.[ Extern.func ([], Some I64) opaque_fn ] in
  let storage = invoke ~imports ~storage ~argument code in
  Alcotest.(check Hexdump.hex) "Same" (Hexdump.of_string "25 00 00 00 00 00 00 00") storage;
...
let test_extern_manipulating_memory () =
  ...
  let mock_hash memory args =
    ...
    in
    let payload, size, target =
      match Value.to_int32 payload, Value.to_int32 size, Value.to_int32 target with
    ...
    let hash = Hexdump.of_string "8d969eef6ecad3c29a3a629280e686cf0c3f5d5a86aff3ca12020c923adc6c92" in
  ...
  Alcotest.(check Hexdump.hex) "Same"
    (Hexdump.of_string "8d969eef6ecad3c29a3a629280e686cf0c3f5d5a86aff3ca12020c923adc6c92")
    storage
...
let test_memory_export () =
  ...
  let storage = Bytes.empty in
  let argument = Bytes.empty in
  Alcotest.check_raises "Invocation error" Invocation_error
    (fun () -> ignore (invoke ~storage ~argument code))
...
let test =
  ...
  "Runtime", [ test_case "Simple invocation" `Quick test_simple_invocation
             ...
             ; test_case "External memory manipulation" `Quick test_extern_manipulating_memory
